        ...
        <View className="gap-5">
          <View>
            ...
            <View className="flex-row flex-wrap gap-2">
              {activities.map((item) => (
                <Pressable key={item} className={`rounded-full px-3.5 py-2.5 ${activity === item ? "bg-brand" : "border border-line bg-surface"}`} onPress={() => setActivity(item)}>
          ...
          <TextInput
            className="h-[56px] rounded-[16px] border border-line bg-surface px-4 text-[16px] text-ink"
            value={title}
          ...
          <View className="flex-row gap-3">
            <TextInput
              className="h-[56px] flex-1 rounded-[16px] border border-line bg-surface px-4 text-[16px] text-ink"
              value={startsAt}
            ...
            />
            <TextInput
              className="h-[56px] w-[92px] rounded-[16px] border border-line bg-surface px-4 text-[16px] text-ink"
              value={spots}
          ...
          <View>
            ...
            <View className="flex-row flex-wrap gap-2">
              {dates.map((date) => (
                <Pressable key={date} className={`rounded-[14px] px-3 py-3 ${dateLabel === date ? "bg-brand-soft" : "bg-surface"}`} onPress={() => setDateLabel(date)}>
          ...
          <View>
            ...
            <View className="flex-row gap-2">
              {levels.map((item) => (
                <Pressable key={item} className={`flex-1 rounded-[14px] px-3 py-3 ${level === item ? "bg-brand-soft" : "bg-surface"}`} onPress={() => setLevel(item)}>
          ...
          <TextInput
            className="h-28 rounded-[16px] border border-line bg-surface p-4 text-[16px] text-ink"
            value={description}
