  ...
  return (
    <View className="rounded-[22px] border border-line bg-surface p-4">
      <View className="mb-3 flex-row items-start justify-between gap-3">
        ...
        <View className={`rounded-full px-3 py-1.5 ${request.status === "pending" ? "bg-brand-soft" : "bg-line"}`}>
      ...
      <Text className="mt-3 rounded-[16px] bg-canvas px-3 py-3 text-[13px] leading-5 text-muted">{request.message}</Text>
      {request.status === "pending" ? (
        <View className="mt-4 flex-row gap-3">
          ...
          <Pressable disabled={pending} className="h-11 flex-1 items-center justify-center rounded-full border border-line bg-surface" onPress={() => onRespond("declined")}>
